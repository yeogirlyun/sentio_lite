#!/usr/bin/env python3
"""
Sentio Lite - Intraday Warmup Optuna Optimization

Strategy:
1. Use intraday 50-bar warmup (bars 1-50 of test day)
...
class IntradayWarmupOptimizer:
    def __init__(self, test_dates: List[str], n_trials: int = 200):
        ...
        print(f"=" * 80)
        print(f"Intraday Warmup Optuna Search")
        print(f"=" * 80)
        print(f"  Test dates: {', '.join(test_dates)}")
        print(f"  Warmup: {self.warmup_bars} bars (intraday, bars 1-50)")
        print(f"  Simulation: 0 days (warmup-only mode)")
        print(f"  Trials: {n_trials}")
        print(f"  Total runs per trial: {len(test_dates)} days")
        print(f"  Data per day: Just 1 day (391 bars)")
        print(f"=" * 80)
    ...
    def run_optimization(self):
        ...
        study = optuna.create_study(
            ...
            study_name='intraday_warmup_optimization'
        )
    ...
    def save_best_config(self, params: Dict, avg_mrd: float):
        ...
        # Update metadata
        config['description'] = "Intraday Warmup Optimized Parameters (50 bars)"
        config['last_updated'] = datetime.now().strftime("%Y-%m-%d")
        config['optimization_date'] = datetime.now().strftime("%Y-%m-%d")
        config['test_performance'] = {
            'avg_mrd': avg_mrd,
            'test_dates': self.test_dates,
            'warmup_mode': 'intraday_50_bars'
        }
    ...
    def save_detailed_results(self, study: optuna.Study):
        ...
        results_file = "optuna_intraday_results.json"
...
def main():
    parser = argparse.ArgumentParser(description='Intraday Warmup Optuna Optimization')
    ...
    print(f"\n{'='*80}")
    print(f"Sentio Lite - Intraday Warmup Optimization")
    print(f"{'='*80}")
    ...
    # Create optimizer
    optimizer = IntradayWarmupOptimizer(test_dates=test_dates, n_trials=args.trials)
