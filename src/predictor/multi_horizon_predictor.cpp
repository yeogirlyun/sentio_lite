...
MultiHorizonPredictor::MultiHorizonPredictor(const std::string& symbol, const Config& config)
    ...
    sentio::EWRLSPredictor::Config ewrls_config_20bar;
    ewrls_config_20bar.lambda = config_.lambda_20bar;
    ewrls_config_20bar.regularization = 1e-6;
    ewrls_config_20bar.use_adaptive_regularization = true;
    ...
    predictor_20bar_ = std::make_unique<OnlinePredictor>(
        OnlinePredictor::NUM_FEATURES, ewrls_config_20bar);
...
MultiHorizonPredictor::MultiHorizonPrediction MultiHorizonPredictor::predict(
    ...
    double pred_20 = predictor_20bar_->predict(features);
    ...
    result.pred_20bar = calculate_quality(pred_20, uncertainties_[3]);
    ...
    } else if (result.optimal_horizon == 10) {
        result.expected_return = result.pred_10bar.prediction;
        result.expected_volatility = result.pred_10bar.uncertainty;
    } else {  // 20
        result.expected_return = result.pred_20bar.prediction;
        result.expected_volatility = result.pred_20bar.uncertainty;
    }
...
void MultiHorizonPredictor::update(const Eigen::VectorXd& features,
    ...
    // Update 20-bar predictor (if 20 bars have passed)
    if (std::isfinite(return_20bar)) {
        double pred_20 = predictor_20bar_->predict(features);
        double error_20 = return_20bar - pred_20;
        predictor_20bar_->update(features, return_20bar);
        update_uncertainty(3, error_20);
    }
}
...
void MultiHorizonPredictor::reset() {
    ...
    predictor_20bar_->reset();
    prediction_errors_.fill(0.0);
    uncertainties_.fill(config_.initial_uncertainty);
}

std::array<size_t, 4> MultiHorizonPredictor::update_counts() const {
    return {
        predictor_1bar_->update_count(),
        predictor_5bar_->update_count(),
        predictor_10bar_->update_count(),
        predictor_20bar_->update_count()
    };
}
...
int MultiHorizonPredictor::determine_optimal_horizon(const MultiHorizonPrediction& pred) const {
    ...
    double sharpe_20 = pred.pred_20bar.prediction / (pred.pred_20bar.uncertainty * std::sqrt(20.0) + 1e-6);
    ...
    double score_20 = sharpe_20 * pred.pred_20bar.confidence;

    // Prefer 5-bar horizon as default (good balance)
    // Only choose others if significantly better
    if (score_5 >= score_1 * 0.9 && score_5 >= score_10 * 0.9 && score_5 >= score_20 * 0.9) {
        return 5;
    } else if (score_20 > score_1 && score_20 > score_5 && score_20 > score_10) {
        return 20;
